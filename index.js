...
else{try{const r2=await fetch('https://api-inference.huggingface.co/models/microsoft/Phi-3-mini-4k-instruct',{method:'POST',headers:{Authorization:`Bearer ${K}`,'Content-Type':'application/json'},body:JSON.stringify({inputs:`<|system|>رد بنفس لهجة المستخدم وكن مختصراً<|end|><|user|>${t}<|end|><|assistant|>`})});const d=await r2.json();await s(c,d[0]?.generated_text?.split('<|assistant|>').pop()?.trim()||JSON.stringify(d).slice(0,100))}catch(e){await s(c,'خطأ: '+e.message)}}}return new Response('OK')}
