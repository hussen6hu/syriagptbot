...
else{try{const d=await(await fetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${K}`,'Content-Type':'application/json'},body:JSON.stringify({model:'openai/gpt-4o',messages:[{role:'system',content:'رد بنفس لهجة المستخدم، كن مفيداً ومختصراً'},{role:'user',content:t}]})})).json();await s(c,d.choices?.[0]?.message?.content||JSON.stringify(d))}catch(e){await s(c,'خطأ: '+e.message)}}}return new Response('OK')}
