...
else{const d=await(await fetch('https://api.deepseek.com/v1/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${K}`,'Content-Type':'application/json'},body:JSON.stringify({model:'deepseek-chat',messages:[{role:'system',content:'رد بنفس لهجة المستخدم، كن مفيداً ومختصراً'},{role:'user',content:t}]})})).json();
